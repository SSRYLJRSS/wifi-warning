...
WifiStatus getCurrentWifi() {
    ...
    for (DWORD i = 0; i < interfaces->dwNumberOfItems; ++i) {
        ...
        if (result == ERROR_SUCCESS && attrs) {
            std::unique_ptr<WLAN_CONNECTION_ATTRIBUTES, decltype(&WlanFreeMemory)> attrsGuard(attrs, WlanFreeMemory);
...
std::vector<std::string> getKnownWifiProfiles() {
    ...
    DWORD negotiated = 0;
    WlanHandle wlan;
    if (WlanOpenHandle(2, nullptr, &negotiated, &wlan.handle) != ERROR_SUCCESS) return profiles;

    PWLAN_INTERFACE_INFO_LIST interfaces = nullptr;
    if (WlanEnumInterfaces(wlan.handle, nullptr, &interfaces) != ERROR_SUCCESS || !interfaces) return profiles;
    std::unique_ptr<WLAN_INTERFACE_INFO_LIST, decltype(&WlanFreeMemory)> interfaceGuard(interfaces, WlanFreeMemory);

    for (DWORD i = 0; i < interfaces->dwNumberOfItems; ++i) {
        PWLAN_PROFILE_INFO_LIST list = nullptr;
        if (WlanGetProfileList(wlan.handle, &interfaces->InterfaceInfo[i].InterfaceGuid, nullptr, &list) != ERROR_SUCCESS || !list) continue;
        std::unique_ptr<WLAN_PROFILE_INFO_LIST, decltype(&WlanFreeMemory)> profileGuard(list, WlanFreeMemory);
